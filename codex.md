<!-- 入口：OpenAI ChatGPT Codex / Codex CLI（极薄壳 v1.1）。权威指令源：README.md。 -->

# Webnovel Writer — Codex 入口壳
...
- Codex 用户：读取 `skills/<命令>/SKILL.md` 严格按其中的 Bash / Read / Write 步骤执行。
- 子代理调用：用平台的子任务工具，或读取 `agents/<代理>.md` 后"扮演"该角色；
  **不得由主流程口头替代子代理的真实输出**，也不得伪造审查 JSON。
- 运行护栏无 hook 自动拦截时：手动执行 `preflight` / `doctor` 等效校验（见 `README.md` §9）。
