...
export SKILL_ROOT="${WEBNOVEL_ROOT:-$PWD}/skills/webnovel-query"
export PROJECT_ROOT="$(python "${SCRIPTS_DIR}/webnovel.py" --project-root "${WORKSPACE_ROOT}" where)"
```

- `PROJECT_ROOT` 必须包含 `.webnovel/state.json`
- **禁止**在 `${WEBNOVEL_ROOT:-$PWD}/` 下读取或写入项目文件
...
按查询类型按需加载，先识别再加载。路径说明：`${WEBNOVEL_ROOT}/skills/webnovel-query/references/` 指 skill 私有 `skills/webnovel-query/references/`；`${WEBNOVEL_ROOT}/references/` 指共享 references。

| 查询类型 | Reference | 实际路径 |
|---------|-----------|---------|
| 数据流 / 优先级 | 数据流规范 | `${SKILL_ROOT}/references/system-data-flow.md` |
| 伏笔分析 | 伏笔分析 | `${SKILL_ROOT}/references/advanced/foreshadowing.md` |
| 节奏分析 | Strand 模式 | `${SKILL_ROOT}/${WEBNOVEL_ROOT}/references/shared/strand-weave-pattern.md` |
