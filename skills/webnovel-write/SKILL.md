...
export SKILL_ROOT="${WEBNOVEL_ROOT:-$PWD}/skills/webnovel-write"
...
Use your platform's agent/task tool to run the agent defined in `agents/context-agent.md` (if a sub-agent tool is unavailable, read that file and adopt its persona inline).
...
Use your platform's agent/task tool to run the agent defined in `agents/reviewer.md` (if a sub-agent tool is unavailable, read that file and adopt its persona inline).
...
`${WEBNOVEL_ROOT}/skills/webnovel-write/references/polish-guide.md` 区段读：先 `Grep` 匹配 `^#{1,3} ` 定位锚点行号，再 `Read` 的 offset/limit 取段——主路径取 `## 2. 执行顺序（必须按序）`；Anti-AI 终检单独区段取 `## 2A. Anti-AI 检测细则` 与 `## Phase 1 增补：Anti-AI 规范（7层，原版）`（词库段），不全文读。`${WEBNOVEL_ROOT}/skills/webnovel-write/references/writing/typesetting.md`、`${WEBNOVEL_ROOT}/skills/webnovel-write/references/style-adapter.md` 短文件，全文读。
...
Use your platform's agent/task tool to run the agent defined in `agents/data-agent.md` (if a sub-agent tool is unavailable, read that file and adopt its persona inline).
