...
路径说明：`${WEBNOVEL_ROOT}/skills/webnovel-init/references/` 指 `skills/webnovel-init/references/`；`${WEBNOVEL_ROOT}/references/` 指共享 references。详细采集字段见 `${WEBNOVEL_ROOT}/skills/webnovel-init/references/init-collection-schema.md`（按需区段读，逐项收集，必填项以「充分性闸门」为准）。

| Step | Trigger | Reference |
|------|---------|-----------|
| Step 1 | always | `${WEBNOVEL_ROOT}/skills/webnovel-init/references/system-data-flow.md`、`${WEBNOVEL_ROOT}/skills/webnovel-init/references/genre-tropes.md` |
| 题材/卖点采集 | always | `${WEBNOVEL_ROOT}/references/genre-profiles.md`（只读当前 genre 段） |
| 角色卡顿 | 人物扁平 | `${WEBNOVEL_ROOT}/skills/webnovel-init/references/worldbuilding/character-design.md` |
| 世界观/力量 | 按需 | `${WEBNOVEL_ROOT}/skills/webnovel-init/references/worldbuilding/faction-systems.md`、`${WEBNOVEL_ROOT}/skills/webnovel-init/references/worldbuilding/power-systems.md`、`${WEBNOVEL_ROOT}/skills/webnovel-init/references/worldbuilding/world-rules.md`、`${WEBNOVEL_ROOT}/skills/webnovel-init/references/worldbuilding/setting-consistency.md` |
| 创意约束 | Step 6 | `${WEBNOVEL_ROOT}/skills/webnovel-init/references/creativity/creativity-constraints.md`（区段：采集读 `## 一、创意包 Schema (Idea Package)`、`## 六、硬约束驱动创意 (Hard Constraints)`、`## 八、评分系统 (Scoring System)`，评分展示读 `### 8.1 五维评分`）、`${WEBNOVEL_ROOT}/skills/webnovel-init/references/creativity/selling-points.md`（区段：`## 9. 核心卖点定位模板` 骨架，按需补 `### 1.3 核心卖点黄金公式`、`## 7. 实战检查清单`）；复合题材读 `creative-combination.md`；卡顿读 `inspiration-collection.md`；题材命中读 `anti-trope-*.md` |
...
- `Agent`：拆分并行子任务；Step 1.5 用户选择参考书拆解作灵感来源时调用 `deconstruction-agent`。
...
- 加载最小参考：`${WEBNOVEL_ROOT}/skills/webnovel-init/references/system-data-flow.md`、`${WEBNOVEL_ROOT}/skills/webnovel-init/references/genre-tropes.md`；`templates/genres/` 仅在选定题材后按需读取。
...
```text
Use your platform's agent/task tool to run the agent defined in `agents/deconstruction-agent.md` (if a sub-agent tool is unavailable, read that file and adopt its persona inline).
...
5. 展示五维评分（详见 `${WEBNOVEL_ROOT}/skills/webnovel-init/references/creativity/creativity-constraints.md` 的 `8.1 五维评分`）辅助决策。
6. 用户选择最终方案，或拒绝并给出原因。
...
- 禁止在插件目录（`${WEBNOVEL_ROOT:-$PWD}`）下生成项目文件；禁止直接把 `WORKSPACE_ROOT` 当作 `PROJECT_ROOT`，除非用户明确指定当前目录就是书项目根。
...
参数全部来自上面的采集对象（书名/题材/主角/金手指/世界观/反派/创意约束等），逐字段映射为 `webnovel.py init` 的 `--*` 选项；完整字段清单见 `${WEBNOVEL_ROOT}/skills/webnovel-init/references/init-collection-schema.md`，可用 `python "${SCRIPTS_DIR}/webnovel.py" init --help` 核对选项名。
