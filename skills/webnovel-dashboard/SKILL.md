...
if [ -z "${WEBNOVEL_ROOT:-$PWD}" ] || [ ! -d "${WEBNOVEL_ROOT:-$PWD}/dashboard" ]; then
  echo "ERROR: 未找到 dashboard 模块: ${WEBNOVEL_ROOT:-$PWD}/dashboard" >&2
  exit 1
fi

export DASHBOARD_DIR="${WEBNOVEL_ROOT:-$PWD}/dashboard"
...
if [ -n "${PYTHONPATH:-}" ]; then
  export PYTHONPATH="${WEBNOVEL_ROOT:-$PWD}:${PYTHONPATH}"
else
  export PYTHONPATH="${WEBNOVEL_ROOT:-$PWD}"
fi
